...
For a single-area audit, use this two-layer output:
...
For broad, multi-area, or parallel audits, create per-area JSON files. This is mandatory unless the user explicitly asks for one combined file:
...
Only create files for areas that were actually audited. If more than one auditor runs, do not create one large `findings.json` unless the user asks for a single combined file.
...
8. Write each auditor's structured output to `findings/<area>.findings.json` when persisting any audit with more than one area.
